...
const FindByReference = () => {
  ...
  const searchHandler = (e) => {
    ...
    findData = ReferenceData.filter((search) => {
      return (
        search.cin.toLowerCase().includes(searchValue.toLowerCase())
      );
    });
        ...
        <Grid item xs={8} md={12} >
          <Item >
            <Grid container spacing={1}  sx={{ml:5,pt:5,pb:5}}>
              ...
              <Grid item xs={3} md={1}  >
                <Typography sx={{px:2,py:1,background:'#F0F0FA',borderRadius:'5px'}} onClick={searchHandler} >
                  Search
                </Typography>
              </Grid>
            </Grid>
...
export default FindByReference

FindByReference.getLayout = function pageLayout(page) {
