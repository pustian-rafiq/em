...
function createData(no,company,date, total_generated) {
  return {
    no,company,date, total_generated
  };
}
...
const GroupList = () => {
  ...
  const tableDatas = rows.filter((search) => {
    return (
      search.company.toLowerCase().includes(searchText.toLowerCase()) 
    ...
    .map((row, index) => {
      ...
      return (
        ...
        >
          ...
          <TableCell className="tableBorder" align="left">{row.company}</TableCell>
          <TableCell className="tableBorder" align="left">{row.date}</TableCell>
          <TableCell className="tableBorder" align="left">{row.total_generated}</TableCell>
          <TableCell className="tableBorder " align="left"  >
            <Link href={`/admin-dashboard/employee/all-print/${row.no}/`} passHref>
              <Typography
                variant='span' title='Print ID Card' component='span' sx={{ background: '#41C457', borderRadius:'3px', fontSize: '17px', mr:1, padding: '5px 10px', color: '#fff',cursor:'pointer' }}>
...
GroupList.getLayout = function pageLayout(page) {
