...
const refund = () => {
  ...
  const data = tableDatas.map((row) => {
    return (
      <Paper elevation={5} sx={{ padding: 3, textAlign: "center", mb: 2 }}>
        <Box
          sx={{
            display: "flex",
    ...
    <Box>
      ...
      <Box sx={{ textAlign: "center" }}>
        ...
        >
          ...
          <InputBase sx={{ ml: 1, flex: 1 }} placeholder="Search" onChange={(e) => setSearchText(e.target.value)}
            value={searchText} />
          <IconButton onClick={searchTextHandler} type="submit" sx={{ p: "10px" }} aria-label="search">
            <Clear  />
          </IconButton>
        </Paper>
        <Button onClick={searchTextHandler} variant="contained" sx={{ width: "200px" }}>
          Refresh
        </Button>
        <Divider sx={{ my: 2 }} />
      </Box>
...
refund.getLayout = function pageLayout(page) {
