...
const rows = [
  { sl: 1, title: "Ehsan Map", url: 12 },
  { sl: 2, title: "Ehsan Blog", url: 43 },
  { sl: 3, title: "Ehsan News", url: 6 },
  { sl: 4, title: "Ehsan Software", url: 8 },
  { sl: 5, title: "Ehsan Marketing", url: 11 },
  {
    sl: 6,
    title: "World Ehsan Email",
    url: 5,
  },
  { sl: 7, title: "Ehsan Live (Upcoming…)", url: 6 },
  { sl: 8, title: "Ehsan RTC (Upcoming…)", url: 34 },
  { sl: 9, title: "Ehsan Calling (Upcoming…)", url: 23 },
  {
    sl: 10,
...
const incentiveWise = () => {
  ...
  const tableDatas = rows.filter((search) => {
    return (
      search.url.toString().toLowerCase().includes(searchText.toLowerCase()) ||
    ...
    .map((row, index) => {
      ...
      return (
        ...
        >
          ...
          >
            <Link
              href={`/admin-dashboard/footer/edit-footer-link/${row.sl}/`}
              passHref
            >
              <Typography
                variant="span"
...
export default incentiveWise;

incentiveWise.getLayout = function pageLayout(page) {
